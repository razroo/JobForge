...
- Anything requiring the full JobForge context (tracker, scoring model, CV match). The orchestrator should have already distilled context down to the input you need.
- Any action that writes to disk, modifies state, or invokes MCP tools.

## Context note

Even though you technically see the global `instructions` context (AGENTS.harness.md, modes/_shared.md, cv.md), **you should ignore it unless the orchestrator explicitly tells you to use it.** Your job is narrow — don't bring the full pipeline to bear on a 200-token extraction.
